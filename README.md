...
**Three channels, not just phone calls:** a real service desk fields email and chat too, and each calls for a genuinely different response, not just the same explanation in a different box. `TrainingScenario.channel` (`PHONE` / `EMAIL` / `CHAT`) drives both the transcript UI (a phone call reads as a transcript, an email shows a subject line and a proper written body, a chat shows a fast, casual message thread) and the written-response grading rubric below it. The triage question and multiple-choice reveal are identical across channels, that's the same skill regardless of how the report arrived, only the reply format and its grading criteria change.
...
app/
  ...
  shift/
    [id]/page.tsx                  # Shift Mode: 5 rounds back-to-back, no retries, ends in a summary
prisma/
