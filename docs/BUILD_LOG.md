...
## Stage 36: Knowledge Management, the fifth ITIL process

A direct follow-up question after Reports: what else was still missing from a complete simulation? Knowledge Management, ITIL's own process for documenting and reusing fixes, had no presence in the app at all. It's a genuinely different thing from Problem Management's Known Error, not a duplicate of it: a Known Error only exists once a formal root-cause investigation is already underway, while most of what a real service desk writes up (how to map a network drive, how to request VPN access) never needed one at all, it's just a common question worth answering once.

**`/knowledge-base`, agent and manager only, never customer-facing**, same reasoning as Problem/Change. A `KnowledgeArticle` has a symptoms/solution pair (what it looks like, how to fix it), a category, and a lifecycle: `DRAFT` (being written, not searchable or linkable yet), `PUBLISHED` (live and reusable), `RETIRED` (outdated, kept on record rather than deleted, and can be re-published). Deliberately no manager-only gate on publish/retire, unlike Problem/Change's close step: maintaining an article is ordinary content upkeep, not a governance decision needing sign-off. Also deliberately no comment thread like every other process has, a knowledge base article isn't a live incident worked by multiple people at once, it's closer to a wiki page; its activity trail (CREATED, EDITED, PUBLISHED, RETIRED, INCIDENT_LINKED) covers what actually needs recording.

**Two links into the rest of the app, in each direction.** An article can optionally be written up `?fromProblemId=` a resolved Problem, the same URL-param convention Change already uses from Problem, pre-filling the draft from that problem's description and workaround/root cause. And the real payoff, the same idea as Problem's workaround-linking: once an agent links a *published* article to an open incident, its solution text shows up directly on that ticket's page, and the assignee gets notified if someone else made the link. A draft can't be linked, only published articles are eligible, so a ticket never ends up pointing at unreviewed advice.

**Reports** picked up two more stats in the "across the other processes" summary row: published article count and draft-awaiting-publish count, computed live from the same table, no separate query path.

**Seeded one worked example** (`KB000001`, published, written up from the existing `PRB000001` POS-terminal problem) plus one left as a `DRAFT` (`KB000002`, a VPN-access how-to), so both halves of the "drafts awaiting publish" vs. "published and searchable" distinction are visible on a fresh sign-in, not just the happy path.

**Verified with Playwright**, signed in as the L3 agent: confirmed the nav link and list page group drafts separately from results, confirmed KB000001's detail page renders its symptoms/solution/source-problem link correctly, linked it to an open incident and confirmed the incident page then showed the documented solution text directly, confirmed the link dropdown only ever offered published articles (never the draft), wrote a new draft end to end and published it, and confirmed a customer sign-in gets redirected away from `/knowledge-base` entirely. Caught and fixed one real bug in this same pass: the incident audit-trail entry for a new link read `KNOWLEDGE_ARTICLE_LINKED`, and the existing activity-label rendering (`activity.type.replace("_", " ")`) only replaces the *first* underscore, so it displayed as "KNOWLEDGE ARTICLE_LINKED" instead of readable text, the first activity type in the app with two underscores instead of one. Renamed the enum value to `ARTICLE_LINKED` (matching the existing `PROBLEM_LINKED`/`INCIDENT_LINKED` naming shape) rather than patching the shared display helper, fixed with a direct `ALTER TYPE ... RENAME VALUE` against the database so no data was lost. All test articles, links, and notifications created during this verification pass were removed from the live database afterward.
